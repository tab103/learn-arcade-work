...
class MyGame(arcade.Window):
    ...
    def __init__(self, width, height, title):
        ...
        self.bucket = arcade.load_sound("collect.wav")
        self.bucket_player = None
    ...
    def setup(self):
        ...
        # Set up the player
        self.player_sprite = arcade.Sprite("EricCartman.png",
                                           scale=0.15)  # Wikipedia (https://en.wikipedia.org/wiki/Eric_Cartman)
        ...
        for x in range(100, 1500, 250):
            for y in range(0, 1400, 70):
                # Randomly skip a box so the player can find a way through
                # DO NOT COPY FOR SUBMITTED LAB
                if random.randrange(8) > 2:
                    wall = arcade.Sprite("wall.jpg", 0.4)
                    # https://www.freepik.com/premium-vector/flat-cartoon-red-brick-wall-background-texture-pattern-continuous-replicate_41785279.htm
                    wall.center_x = x
                    wall.center_y = y
                    self.wall_list.append(wall)

        for x in range(50, 1550, 110):
            wall = arcade.Sprite("wall.jpg", 0.4)
            wall.center_x = x
            wall.center_y = 0
            self.wall_list.append(wall)
        ...
        for i in range(BUCKET_COUNT):
            bucket = arcade.Sprite("KfcBucket.png", scale=0.1)  # https://pngimg.com/image/82059#google_vignette
            bucket_placed_successfully = False
            while not bucket_placed_successfully:
                # Position the coin
                bucket.center_x = random.randrange(100, 1500)
                bucket.center_y = random.randrange(0, 1400)

                wall_hit_list = arcade.check_for_collision_with_list(bucket, self.wall_list)
                bucket_hit_list = arcade.check_for_collision_with_list(bucket, self.bucket_list)

                if len(wall_hit_list) == 0 and len(bucket_hit_list) == 0:
                    bucket_placed_successfully = True
    ...
    def on_update(self, delta_time):
        ...
        # Calculate speed based on the keys pressed
            ...
            if self.left_pressed and not self.right_pressed:
                ...
                self.player_sprite.change_x = PLAYER_MOVEMENT_SPEED
        bucket_hit_list = arcade.check_for_collision_with_list(self.player_sprite, self.bucket_list)
        for bucket in bucket_hit_list:
            bucket.remove_from_sprite_lists()
            self.score += 1
            if not self.bucket_player or not self.bucket_player.playing:
                self.bucket_player = arcade.play_sound(self.bucket)
